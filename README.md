...
仓库已包含预构建的 `history_index.db`，直接执行：
...
- `.trae/skills/tangshan-yizhong-history/history_index.db`：预构建本地索引
- `Makefile`：仓库根快捷命令入口
...
- 默认直接使用仓库中已提交的 `history_index.db`
- 只有在你想重建索引时，才需要仓库根目录下的 `唐山市第一中学校史.pdf`
- 本地生成的 `.venv` 和 PDF 原件默认不会提交到仓库
